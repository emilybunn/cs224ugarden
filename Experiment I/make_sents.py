...
def get_components_npz(filename='npz', sent_type='gp', context=False, extension=False, comma=False):
    ...
    subj_ind = [len(df.loc[i, 'Start'].split()) - 1 for i in range(len(df['Start']))]
...
def get_components_nps(filename='nps', sent_type='gp', context=False, extension=False, that=False):
    ...
    # comma can be added to negate GP effect
    trigger = 7
    subj_ind = [len(df.loc[i, 'Subject'].split()) - 1 for i in range(len(df['Subject']))]
...
def get_components_va(filename='vawip', sent_type='gp', unreduced=False, intervener=False):
    ...
    for i in range(len(df)):
        ...
        post.append(s)
            # f.write(s)
    return pre, post, [1 for i in range(len(df))]
...
def make_sents_na(filename='vawip', sent_type='gp', that=False, extension=False):
...
def main():
    ...
    #write_sents(make_sents_va(), filename='vawip')
    #write_sents(make_sents_nps(), filename='nps')
    #write_sents(make_sents_npz(), filename='npz')
    get_components_nps()
